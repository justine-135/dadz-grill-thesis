...
class Foods extends Dbh{
    ...
    protected function setFood($name, $group, $cost, $stats, $fileActualExt, $fileTempLoc, $target, $fileError, $fileNameTime, $allowed){
        if(in_array($fileActualExt, $allowed)){
            if($fileError === 0){
            if(move_uploaded_file($fileTempLoc, $target)){
                $sql = "INSERT INTO inventory (item_name, item_group, cost, photo, order_status)
                VALUES (?, ?, ?, ?, ?)";
                
                $stmt = $this->connection()->prepare($sql);
                $stmt->execute([$name, $group, $cost, $fileNameTime, $stats]);
            }
            }
        }
    }

    protected function updateFood($name, $group, $cost, $stats, $fid){
        $sql = "UPDATE inventory SET item_name = '$name', item_group = '$group', cost = $cost, order_status = '$stats' WHERE fid = $fid";
        $stmt = $this->connection()->prepare($sql);
        $stmt->execute();
    }
